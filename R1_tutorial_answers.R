...
names(texts) <- c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday")
...
mean(weekday_texts) < mean(weekend_texts)
...
emails_category <- factor(emails_category, order = TRUE, levels = c("low", "high"))
emails_category
...
dog_data <- read.csv("/Users/sgg24/Downloads/dog_intelligence.csv")
...
subset(dog_data, Breed == "Puli") # or
dog_data[dog_data$Breed == "Puli",]
